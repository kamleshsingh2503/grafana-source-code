import { CellProps } from 'react-table';
import { Field } from '@grafana/data';
import { TableStyles } from './styles';
import { FC } from 'react';
export interface TableFieldOptions {
    width: number;
    align: FieldTextAlignment;
    displayMode: TableCellDisplayMode;
    hidden?: boolean;
}
export declare enum TableCellDisplayMode {
    Auto = "auto",
    ColorText = "color-text",
    ColorBackground = "color-background",
    GradientGauge = "gradient-gauge",
    LcdGauge = "lcd-gauge",
    JSONView = "json-view"
}
export declare type FieldTextAlignment = 'auto' | 'left' | 'right' | 'center';
export interface TableRow {
    [x: string]: any;
}
export declare type TableFilterActionCallback = (key: string, value: string) => void;
export declare type TableColumnResizeActionCallback = (fieldIndex: number, width: number) => void;
export declare type TableSortByActionCallback = (state: TableSortByFieldState[]) => void;
export interface TableSortByFieldState {
    fieldIndex: number;
    desc?: boolean;
}
export interface TableCellProps extends CellProps<any> {
    tableStyles: TableStyles;
    field: Field;
}
export declare type CellComponent = FC<TableCellProps>;
